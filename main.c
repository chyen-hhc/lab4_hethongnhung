...
void app_main(void)
{
        ...
        xTaskCreate(TaskReception, "Reception task", 2048, NULL, 1, NULL);
        for (size_t i = 0; i < TaskFunction_NUM; i++)
                xTaskCreate(TaskFunction, "Functional task", 2048, (void *)i, 1, &FunctionalTaskHandle[i]);
        xTaskCreate(TaskForCleanGarbageRequest, "Cleanup task", 2048, NULL, 1, NULL);
        vTaskDelete(NULL);
}
